...
def filter_and_merge(bedgraph, outfile, logger, cov_threshold=2):
    filter_and_merge_command = f"awk '$4>={cov_threshold}' {bedgraph}  | bedtools merge -i - -c 4,5,6 -o distinct,distinct,distinct > {outfile}"
...
def coverage_main(args):
    ...
    try:
        ...
        if strand == 'unstranded':
            ...
            filter_and_merge(bedgraph_file, filtered_bed, logger)
        else:
        ...
        if not os.path.exists(for_filtered_bed): 
            filter_and_merge(for_bedgraph_file,for_filtered_bed,logger)
        ...
        if not os.path.exists(rev_filtered_bed): 
            filter_and_merge(rev_bedgraph_file,rev_filtered_bed,logger)
        else:
